...
int sr_leasetime_cb(sr_ctx_t *ctx, sr_change_oper_t op, sr_val_t *old_val,
...
int uci_leasetime_cb(sr_ctx_t *ctx, char *xpath, char *ucipath,
...
int sr_stop_cb(sr_ctx_t *ctx, sr_change_oper_t op, sr_val_t *old_val,
               sr_val_t *new_val, char *xpath, char *ucipath) {
  int rc = SR_ERR_OK;
  sr_val_t *value = NULL;
...
int uci_stop_cb(sr_ctx_t *ctx, char *xpath, char *ucipath, sr_edit_flag_t flag,
                void *data) {
  int rc = SR_ERR_OK;
  int uci_ret = UCI_OK;
  char *key = NULL;
  ...
  if (UCI_ERR_NOTFOUND == uci_ret) {
    ...
    UCI_CHECK_RET(uci_ret, &rc, cleanup, "get_uci_item %d %s", uci_ret,
                  start_ucipath);
...
int uci_dhcpv6_option_cb(sr_ctx_t *ctx, char *xpath, char *ucipath,
                         sr_edit_flag_t flag, void *data) {
  int rc = SR_ERR_OK;
...
cleanup:
  ...
  return true == check_passed ? uci_option_cb(ctx, xpath, ucipath, flag, data)
                              : SR_ERR_OK;
}

/* parse only interfaces with option proto 'dhcpv6' */
int uci_dhcpv6_boolean_cb(sr_ctx_t *ctx, char *xpath, char *ucipath,
                          sr_edit_flag_t flag, void *data) {
  int rc = SR_ERR_OK;
  char *uci_val = NULL, *if_ucipath = NULL;
  bool check_passed = false;
...
cleanup:
  ...
  return true == check_passed ? uci_boolean_cb(ctx, xpath, ucipath, flag, data)
                              : SR_ERR_OK;
}

/* trnasform uci list to sysrepo leaf */
int sr_list_option_cb(sr_ctx_t *ctx, sr_change_oper_t op, sr_val_t *old_val,
                      sr_val_t *new_val, char *xpath, char *ucipath) {
  int rc = SR_ERR_OK;
  int uci_ret = UCI_OK;
  struct uci_ptr ptr = {};
...
void dhcp_v6_cb(struct ubus_request *req, int type, struct blob_attr *msg) {
  ...
  json_object_object_foreach(o, tmp_interface_key, tmp_interface_val) {
    ...
    if (NULL != tmp_interface_key && NULL != tmp_interface_val) {
      /* get list of leases */
      json_object_object_foreach(tmp_interface_val, tmp_device_key,
                                 tmp_device_val) {
        if (NULL != tmp_device_key && NULL != tmp_device_val) {
          ...
          int device_element;
          for (device_element = 0; device_element < device_len;
               device_element++) {
            ...
            if (NULL != i) {
              ...
              CHECK_NULL_MSG(
                  i, &rc, cleanup,
                  "could not get json object ipv6-addr or ipv6-prefix");
              counter = counter + json_object_array_length(i);
            }
          }
        }
    ...
    if (NULL != interface_key && NULL != interface_val) {
      ...
      json_object_object_foreach(interface_val, device_key, device_val) {
        if (NULL != device_key && NULL != device_val) {
          ...
          for (device_element = 0; device_element < device_len;
               device_element++) {
            struct json_object *val = NULL, *v = NULL, *jobj_assigned = NULL;
            long number = 0;
            char *string_number = NULL;
            val = json_object_array_get_idx(device_val, device_element);

            json_object_object_get_ex(val, "assigned", &jobj_assigned);
            CHECK_NULL_MSG(jobj_assigned, &rc, cleanup,
                           "could not get json object assigned");
            const char *assigned = json_object_get_string(jobj_assigned);

            int len = strlen(assigned) + 76;
            xpath = malloc(sizeof(char) * len);
            CHECK_NULL_MSG(xpath, &rc, cleanup, "malloc failed");

            json_object_object_get_ex(val, "hostname", &v);
            CHECK_NULL_MSG(v, &rc, cleanup,
                           "could not get json object hostname");
            snprintf(xpath, len,
                     "/terastream-dhcp:dhcp-v6-leases/"
                     "dhcp-v6-lease[assigned='%s']/hostname",
                     assigned);
            rc = sr_val_set_xpath(&sr_val[counter], xpath);
            CHECK_RET(rc, cleanup, "failed sr_val_set_xpath: %s",
                      sr_strerror(rc));
            rc = sr_val_set_str_data(&sr_val[counter], SR_STRING_T,
                                     (char *)json_object_get_string(v));
            CHECK_RET(rc, cleanup, "failed sr_val_set_str_data: %s",
                      sr_strerror(rc));
            counter++;

            json_object_object_get_ex(val, "iaid", &v);
            CHECK_NULL_MSG(v, &rc, cleanup, "could not get json object iaid");
            string_number = (char *)json_object_get_string(v);
            number = strtol(string_number, &string_number, 10);
            snprintf(xpath, len,
                     "/terastream-dhcp:dhcp-v6-leases/"
                     "dhcp-v6-lease[assigned='%s']/iaid",
                     assigned);
            rc = sr_val_set_xpath(&sr_val[counter], xpath);
            CHECK_RET(rc, cleanup, "failed sr_val_set_xpath: %s",
                      sr_strerror(rc));
            ...
            json_object_object_get_ex(val, "duid", &v);
            CHECK_NULL_MSG(v, &rc, cleanup, "could not get json object duid");
            snprintf(xpath, len,
                     "/terastream-dhcp:dhcp-v6-leases/"
                     "dhcp-v6-lease[assigned='%s']/duid",
                     assigned);
            rc = sr_val_set_xpath(&sr_val[counter], xpath);
            CHECK_RET(rc, cleanup, "failed sr_val_set_xpath: %s",
                      sr_strerror(rc));
            ...
            string_number = (char *)json_object_get_string(v);
            CHECK_NULL_MSG(string_number, &rc, cleanup,
                           "could not get get string");
            ...
            snprintf(xpath, len,
                     "/terastream-dhcp:dhcp-v6-leases/"
                     "dhcp-v6-lease[assigned='%s']/valid",
                     assigned);
            rc = sr_val_set_xpath(&sr_val[counter], xpath);
            CHECK_RET(rc, cleanup, "failed sr_val_set_xpath: %s",
                      sr_strerror(rc));
            ...
            if (NULL != v) {
              ...
              for (ip_element = 0; ip_element < ip_len; ip_element++) {
                struct json_object *ip =
                    json_object_array_get_idx(v, ip_element);
                snprintf(xpath, len,
                         "/terastream-dhcp:dhcp-v6-leases/"
                         "dhcp-v6-lease[assigned='%s']/ipv6",
                         assigned);
                rc = sr_val_set_xpath(&sr_val[counter], xpath);
                CHECK_RET(rc, cleanup, "failed sr_val_set_xpath: %s",
                          sr_strerror(rc));
                rc = sr_val_set_str_data(&sr_val[counter], SR_STRING_T,
                                         (char *)json_object_get_string(ip));
                CHECK_RET(rc, cleanup, "failed sr_val_set_str_data: %s",
                          sr_strerror(rc));
                counter++;
              }
              ...
              CHECK_NULL_MSG(v, &rc, cleanup,
                             "could not get json object length");
              string_number = (char *)json_object_get_string(v);
              CHECK_NULL_MSG(string_number, &rc, cleanup,
                             "could not get get string");
              number = strtol(string_number, &string_number, 10);
              snprintf(xpath, len,
                       "/terastream-dhcp:dhcp-v6-leases/"
                       "dhcp-v6-lease[assigned='%s']/length",
                       assigned);
              rc = sr_val_set_xpath(&sr_val[counter], xpath);
              CHECK_RET(rc, cleanup, "failed sr_val_set_xpath: %s",
                        sr_strerror(rc));
              ...
              for (ip_element = 0; ip_element < ip_len; ip_element++) {
                ...
                CHECK_NULL_MSG(address, &rc, cleanup,
                               "could not get json object address");
                snprintf(xpath, len,
                         "/terastream-dhcp:dhcp-v6-leases/"
                         "dhcp-v6-lease[assigned='%s']/ipv6",
                         assigned);
                ...
                CHECK_RET(rc, cleanup, "failed sr_val_set_xpath: %s",
                          sr_strerror(rc));
                rc = sr_val_set_str_data(
                    &sr_val[counter], SR_STRING_T,
                    (char *)json_object_get_string(address));
                CHECK_RET(rc, cleanup, "failed sr_val_set_str_data: %s",
                          sr_strerror(rc));
                counter++;
...
int fill_dhcp_v6_data(sr_ctx_t *ctx, char *xpath, sr_val_t **values,
  ...
  u_rc =
      ubus_invoke(u_ctx, id, "ipv6leases", buf.head, dhcp_v6_cb, &sr_values, 0);
  if (UBUS_STATUS_OK != u_rc) {
...
void dhcp_v4_cb(struct ubus_request *req, int type, struct blob_attr *msg) {
  ...
  json_object_object_foreach(r, key, val) {
    ...
    snprintf(
        xpath, len,
        "/terastream-dhcp:dhcp-v4-leases/dhcp-v4-lease[name='%s']/leasetime",
        key);
    rc = sr_val_set_xpath(&sr_val[counter], xpath);
    ...
    snprintf(
        xpath, len,
        "/terastream-dhcp:dhcp-v4-leases/dhcp-v4-lease[name='%s']/hostname",
        key);
    rc = sr_val_set_xpath(&sr_val[counter], xpath);
    CHECK_RET(rc, cleanup, "failed sr_val_set_xpath: %s", sr_strerror(rc));
    rc = sr_val_set_str_data(&sr_val[counter], SR_STRING_T,
    ...
    snprintf(xpath, len,
             "/terastream-dhcp:dhcp-v4-leases/dhcp-v4-lease[name='%s']/ipaddr",
             key);
    rc = sr_val_set_xpath(&sr_val[counter], xpath);
    CHECK_RET(rc, cleanup, "failed sr_val_set_xpath: %s", sr_strerror(rc));
    rc = sr_val_set_str_data(&sr_val[counter], SR_STRING_T,
    ...
    snprintf(xpath, len,
             "/terastream-dhcp:dhcp-v4-leases/dhcp-v4-lease[name='%s']/macaddr",
             key);
    rc = sr_val_set_xpath(&sr_val[counter], xpath);
    CHECK_RET(rc, cleanup, "failed sr_val_set_xpath: %s", sr_strerror(rc));
    rc = sr_val_set_str_data(&sr_val[counter], SR_STRING_T,
    ...
    snprintf(xpath, len,
             "/terastream-dhcp:dhcp-v4-leases/dhcp-v4-lease[name='%s']/device",
             key);
    rc = sr_val_set_xpath(&sr_val[counter], xpath);
    CHECK_RET(rc, cleanup, "failed sr_val_set_xpath: %s", sr_strerror(rc));
    rc = sr_val_set_str_data(&sr_val[counter], SR_STRING_T,
    ...
    snprintf(
        xpath, len,
        "/terastream-dhcp:dhcp-v4-leases/dhcp-v4-lease[name='%s']/connected",
        key);
    rc = sr_val_set_xpath(&sr_val[counter], xpath);
    CHECK_RET(rc, cleanup, "failed sr_val_set_xpath: %s", sr_strerror(rc));
